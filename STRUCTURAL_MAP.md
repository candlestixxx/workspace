# Structural Map — candlestixxx/workspace

| # | Submodule | Path | Remote URL | Current Commit |
|---|-----------|------|-----------|----------------|
| 1 | brokeragentworkflow | brokeragentworkflow/ | https://github.com/candlestixxx/brokeragentworkflow | 024f390 |
| 2 | excel-legacy-leadgen | excel-legacy-leadgen/ | https://github.com/candlestixxx/excel-legacy-leadgen.git | e62c3d0 |
| 3 | explorerexedecompiled | explorerexedecompiled/ | https://github.com/candlestixxx/explorerexedecompiled.git | 2ce2bab |
| 4 | forclosureworkflow | forclosureworkflow/ | https://github.com/candlestixxx/forclosureworkflow.git | 51fea2c |
| 5 | leadG | leadG/ | https://github.com/candlestixxx/leadG.git | 8418f0d |
| 6 | p2p_service_marketplace | p2p_service_marketplace/ | https://github.com/candlestixxx/p2p_service_marketplace.git | 97c8077 |
| 7 | re-agent-workflow-media-1 | re-agent-workflow-media-1/ | https://github.com/candlestixxx/re-agent-workflow-media-1 | e6fe336 |
| 8 | realestatecrm | realestatecrm/ | git@github.com:candlestixxx/realestatecrm.git | c4cd342 |
| 9 | realestateleadcaller | realestateleadcaller/ | https://github.com/candlestixxx/realestateleadcaller.git | e5d5217 |
| 10 | realestateprototype | realestateprototype/ | https://github.com/candlestixxx/realestateprototype.git | 87a5570 |
| 11 | skillzhub | skillzhub/ | https://github.com/candlestixxx/skillzhub.git | eeb7d2f |
| 12 | socialmediacontentplanner | socialmediacontentplanner/ | https://github.com/candlestixxx/socialmediacontentplanner.git | b82dcac |
| 13 | techno_platform_detroit | techno_platform_detroit/ | https://github.com/candlestixxx/techno_platform_detroit.git | 9ef1d14 |
| 14 | theta-data-api | theta-data-api/ | https://github.com/candlestixxx/theta-data-api.git | 1110e9b |
| 15 | ultratrader | ultratrader/ | https://github.com/candlestixxx/ultratrader.git | bdd0ff8 |
| 16 | LegacyLeads | LegacyLeads/ | https://github.com/candlestixxx/LegacyLeads.git | 6b7bd21 |
| 17 | crowdsourced_dance_club | crowdsourced_dance_club/ | https://github.com/candlestixxx/crowdsourced_dance_club.git | b29b0b4 |
| 18 | Prank-Deck-AI | Prank-Deck-AI/ | https://github.com/candlestixxx/Prank-Deck-AI.git | 19de980 |
| 19 | bobgui | bobgui/ | https://github.com/candlestixxx/bobgui.git | e43bc6f51a |
| 20 | hyperharness | hyperharness/ | https://github.com/candlestixxx/hyperharness.git | 9a43bde |

**Notes:**
- All 20 submodules are candlestixxx repos. `ultratrader` was forked from robertpelloni/ultratrader (upstream no longer accessible, dead remote removed).
- `Prank-Deck-AI`, `bobgui`, and `hyperharness` were added in v1.0.19 after being previously removed (empty/clone timeout issues).
- `crowdsourced_dance_club` has a nested submodule `external/auto_dj_script` (robertpelloni/auto_dj_script).
- `hyperharness` requires `--depth 1` for initial clone due to 800MB+ repo size.
- Submodule count: 20 active submodules (plus 1 nested).

Root remote: `https://github.com/candlestixxx/workspace.git` (main branch)

**Last verified:** 2026-07-20 (v1.0.23) — 20 active submodules. Full forward/reverse merge cycle complete.
